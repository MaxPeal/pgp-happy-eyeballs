...
docker run -d \
	...
	tianon/rawdns sh -xec '
		...
		if [ -n "$squignixHostname" ]; then
			for host in "deb.debian.org" "snapshot.debian.org" "archive.ubuntu.com" "dl-cdn.alpinelinux.org" "www.oberhumer.com" "www.zlib.net" "tukaani.org" "ftp.barfooze.de" "github.com"; do 
				cat >> /rawdns.json <<-EOF
